...
def load_vgg(sess, vgg_path):
    ...
    ten_input = graph.get_tensor_by_name(vgg_input_tensor_name)
    ten_keep_prob = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)
    ten_l3 = graph.get_tensor_by_name(vgg_layer3_out_tensor_name)
    ten_l4 = graph.get_tensor_by_name(vgg_layer4_out_tensor_name)
    ten_l7 = graph.get_tensor_by_name(vgg_layer7_out_tensor_name)
    
    
    return ten_input, ten_keep_prob, ten_l3, ten_l4, ten_l7
...
#    custom_config.gpu_options.allow_growth = True
    ...
    with tf.Session(config=custom_config) as sess:
        ...
        # get vgg layers
        vgg_input_image, vgg_keep_prob, vgg_l3, vgg_l4, vgg_l7 = load_vgg(sess, vgg_path)
        
        ## create deconv and skip layers
        decon_final = layers(vgg_l3, vgg_l4, vgg_l7, num_classes)
        
        # initialize optimizer functions
        logits, train_op, cross_entropy_loss = optimize(decon_final, correct_label, learning_rate, num_classes)
